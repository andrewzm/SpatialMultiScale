...
%   For Matern model    
    ...
    for i = 1:nbatches
        ...
        pred_zp(idx) = pred_zp_batch;
        se_zp(idx) = 0;
        SPE(idx) = SPE_batch;

        Cpp=Cov_FSABlock(theta, loc_p(idx,:), loc_p(idx,:), knots, Blockid_p(idx), Blockid_p(idx), funname)+theta.epsilon*eye(1000);
        ...
        Pred_SD(idx)=sqrt(diag(Cpp-(Temp1_Cp-Temp2_Cp*(Q\(Temp2_Cp')))));


    end
    
    % FIX TO RETURN SUM OF SQUARES THEN TAKE MEAN AFTERWARDS
    MSPE = mean(SPE);
